...
add_integration_function('integrate_pre_profile_areas', 'EhPortal_profile_areas');
add_integration_function('integrate_whos_online', 'EhPortal_whos_online');
add_integration_function('integrate_pre_log_stats', 'EhPortal_log_stats');
...
// EhPortal additional tables
$EhPortalTables = array(
	'sp_profiles' => array(
		'columns' => array(
			array(
				'name' => 'id_member',
			...
			array(
				'name' => 'ehportal_ignore_members',
				'type' => 'tinyint',
				'size' => '1',
				'default' => 0,
			),
			array(
				'name' => 'ehportal_enable_audio',
				'type' => 'tinyint',
				'size' => '1',
				'default' => 1,
			),
...
foreach ($EhPortalTables as $table => $data)
{
	if (!check_table_existsEPInstaller($table))
	{
		$smcFunc['db_create_table']('{db_prefix}' . $table, $data['columns'], $data['indexes'], array(), 'ignore', 'fatal');
		$info .= '<li>"' . $table . '" table created.</li>';
	}
...
// Take control of the $db_package_log array()
...
function check_table_existsEPInstaller($table)
{
